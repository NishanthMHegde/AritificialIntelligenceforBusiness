...
class Environment(object):
    ...
    def update_env(self, direction,energy_ai, month):
        ...
        self.current_number_of_users = self.current_number_of_users + np.random.randint(-self.max_update_users, self.max_update_users)
        ...
        self.current_data_rate = self.current_data_rate + np.random.randint(-self.max_update_data, self.max_update_data)
        ...
        if self.temperature_ai < self.min_temperature:
            if (self.train == 1):
                ...
                self.total_energy_ai = self.total_energy_ai + self.optimal_temperature[0] - self.temperature_ai
        elif self.temperature_ai > self.max_temperature:
            if (self.train == 1):
                self.game_over = 1
            else:
                self.temperature_ai = self.optimal_temperature[1]
                self.total_energy_ai = self.total_energy_ai + self.temperature_ai - self.optimal_temperature[0]

        #Updating the scores
        self.total_energy_ai = self.total_energy_ai +  energy_ai
        self.total_energy_noai = self.total_energy_noai +  energy_noai
        ...
        scaled_data_rate = (self.current_data_rate - self.min_data_rate)/(self.max_data_rate - self.min_data_rate)
